...
func update_ship(id: int, cell: Vector2i, facing: int, hp: int, has_cargo: bool, animate := true) -> Tween:
...
func _draw() -> void:
	var board_rect := Rect2(grid_origin, Vector2(grid_size) * cell_size)
	...
	for y in range(grid_size.y + 1):
		var p1 := grid_origin + Vector2(0, float(y) * cell_size)
		var p2 := grid_origin + Vector2(float(grid_size.x) * cell_size, float(y) * cell_size)
		draw_line(p1, p2, Color(1.0, 1.0, 1.0, 0.25), 1.0)
	for x in range(grid_size.x + 1):
		var q1 := grid_origin + Vector2(float(x) * cell_size, 0)
		var q2 := grid_origin + Vector2(float(x) * cell_size, float(grid_size.y) * cell_size)
		draw_line(q1, q2, Color(1.0, 1.0, 1.0, 0.25), 1.0)

	for rock in rocks:
		var rect := Rect2(cell_to_pixel(rock) + Vector2(6, 6), Vector2(cell_size - 12, cell_size - 12))
	...
	for cell in pickup_cells:
		var center := cell_to_pixel(cell) + Vector2.ONE * cell_size * 0.5
	...
	for cell in delivery_cells:
		var center := cell_to_pixel(cell) + Vector2.ONE * cell_size * 0.5
		var rr := Rect2(center - Vector2.ONE * cell_size * 0.17, Vector2.ONE * cell_size * 0.34)
